...
def check_wallet_balance(address, api_key):
    """Checks the balance of a Binance Smart Chain (BNB) address using the BscScan API."""
...
def get_token_balances(address, api_key):
    """Fetches token balances for a Binance Smart Chain address using BscScan API."""
    url = f"https://api.bscscan.com/api"
    params = {
        "module": "account",
        "action": "tokentx",
...
def export_wallets_to_file(wallets, filename="wallets_with_balance.csv"):
    ...
    with open(filename, mode="w", newline="") as file:
        ...
        writer.writerow(["Seed Phrase", "Wallet Address", "Balance (BNB)", "Tokens"])
        writer.writerows(wallets)
    print(f"Exported wallets to {filename}")

def main():
    print("Welcome to the Wallet Balance Checker for Binance Smart Chain (BNB)!")
    ...
    api_key = input("Enter your BscScan API key: ").strip()
